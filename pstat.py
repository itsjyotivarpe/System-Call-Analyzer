...
with conn:
	curs.executescript("""
		...
		CREATE TABLE stat_tab(FileName TEXT, Mode TEXT, Desc TEXT, Size INTEGER);
		""");
	...
	for row in rows:
		if row[2] in stat:
			if row[4] =='0':
				tmp = row[3].split(",")
				filename = tmp[0]
				curs.execute("SELECT FileName FROM stat_tab WHERE FILENAME = '%s';"%(filename))
				row = curs.fetchone()
				...
				if row ==None:
					...
					curs.execute("INSERT INTO stat_tab VALUES('%s','%s','%s','%s');"%(filename, mode, desc, size))

	curs.execute("SELECT * from stat_tab")
	rows = curs.fetchall()
