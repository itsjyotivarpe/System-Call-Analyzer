...
with conn:
	curs.executescript("""
		DROP TABLE IF EXISTS fstat_tab;
		CREATE TABLE fstat_tab(FileName TEXT, Mode TEXT, Description TEXT, Size INTEGER);
		""");
	curs.executescript("""
		DROP TABLE IF EXISTS file_name;
		CREATE TABLE file_name(fid INTEGER, filename TEXT);
		""");
	...
	for row in rows:
	    ...
	    if row[2] == 'open':
	        ...
	        temp = row[3].split(",")
	        filename1 = temp[0]
	        curs1.execute("""INSERT INTO file_name VALUES('%s','%s');"""%(fid1, filename1))

	    elif row[2] == 'close':
	        temp = row[3].split(",")
	        fid1 = temp[0]
	        curs1.execute("""DELETE FROM file_name WHERE fid = '%s';"""%(fid1))

	    elif row[2] in fstat:
		tmp = row[3].split(",")
		fid1 = int(tmp[0])
	        curs1.execute("SELECT * FROM file_name WHERE fid = '%s';"%(fid1))
		row = curs1.fetchone()
		...
		curs.execute("SELECT FILENAME FROM fstat_tab WHERE FileName = '%s';"%(filename))
		row = curs.fetchone()
		...
		if row ==None:
			...
			curs.execute("INSERT INTO fstat_tab VALUES('%s','%s','%s','%s');"%(filename, mode, desc, size))

	curs1.execute("DROP table file_name;")

	'''
	curs.execute("SELECT * from fstat_tab")
	rows = curs.fetchall()
